    ...
    <div className='mt-2'>
      {
        finalConnectionOther.map((lel: any)=>(
            <Link href={`/message/${lel.userId}`} key={lel} className="border p-2 bg-teal-400 gap-4 flex flex-row items-center shadow mb-2">
              <div className="w-14 h-14 rounded-full overflow-hidden bg-gray-400 border-2 border-gray-600">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs md:text-sm font-light text-white">{lel.email}</span>
              </div>
            </Link>
        ))
      }
      {
        finalConnectionConnect.map((lel: any)=>(
            <Link href={`/message/${lel.connectId}`} key={lel} className="border p-2 bg-teal-400 gap-4 flex flex-row items-center shadow mb-2">
              <div className="w-14 h-14 rounded-full overflow-hidden bg-gray-400 border-2 border-gray-600">
                <Image src={lel.profilImg} alt='Bird' width={100} height={100} className='rounded-full'/>
              </div>
              <div className="flex flex-col">
                <span className="text-xs md:text-sm text-white font-bold mb-2">{lel.username}</span>
                <span className="text-xs md:text-sm text-white font-light">{lel.email}</span>
              </div>
            </Link>
        ))
      }
